...
function wpmc_recover_do(items, totalcount) {
	...
	if (items.length > 0) {
		...
		jQuery('#wpmc_progression').html("Done. Please <a href='?page=media-file-cleaner'>refresh</a> this page.");
		return;
	}
	jQuery.post(ajaxurl, data, function (response) {
		reply = jQuery.parseJSON(response);
...
function wpmc_scan_do() {
	...
	if (wpmc.files.length > 0) {
		...
		jQuery('#wpmc_progression').html(wpmc.issues + " issue(s) found. <a href='?page=media-file-cleaner'></span>Refresh</a>.");
		return;
	}
	jQuery.post(ajaxurl, data, function (response) {
		try {
