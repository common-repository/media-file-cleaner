...
function wpmc_wp_ajax_wpmc_delete_do () {
	...
	echo json_encode(
		array(
			...
			'message' => __( "Status unknown.", 'media-file-cleaner' )
		)
	);
	die();
}
...
function wpmc_wp_ajax_wpmc_scan_do () {
	...
	echo json_encode(
		array(
			...
			'message' => __( "Items checked.", 'media-file-cleaner' )
		)
	);
	die();
}
...
function wpmc_wp_ajax_wpmc_get_all_deleted () {
	...
	echo json_encode(
		array(
			...
			'success' => true,
			'message' => __( "List generated.", 'media-file-cleaner' )
		)
	);
	die;
}
...
function wpmc_wp_ajax_wpmc_scan() {
	...
	if ( $uploads ) {
		...
		echo json_encode(
			array(
				'results' => $files, 'success' => true, 'message' => __( "Files retrieved.", 'media-file-cleaner' )
			)
		);
		die();
	}
	...
	if ( $medias ) {
		...
		echo json_encode(
			array( 'results' => $results, 'success' => true, 'message' => __( "Medias retrieved.", 'media-file-cleaner' ) )
		);
		die();
	}

	// No task.
	echo json_encode( array( 'success' => false, 'message' => __( "No task.", 'media-file-cleaner' ) ) );
	die();
}
...
function echo_issue( $issue ) {
	if ( $issue == 'NO_CONTENT' ) {
		_e( "Seems not in use.", 'media-file-cleaner' );
	}
	else if ( $issue == 'NO_MEDIA' ) {
		_e( "Not in Media Library.", 'media-file-cleaner' );
	}
	else if ( $issue == 'ORPHAN_RETINA' ) {
		_e( "Orphan retina.", 'media-file-cleaner' );
	}
	else if ( $issue == 'ORPHAN_MEDIA' ) {
		_e( "File not found.", 'media-file-cleaner' );
	}
	else {
		echo $issue;
	}
...
function wpmc_screen() {
	...
	<div class='wrap'>
		...
		<h1>Media File Cleaner <?php by_jordy_meow(); ?></h1>
		...
		<div style='margin-top: 0px; background: #FFF; padding: 5px; border-radius: 4px; height: 28px; box-shadow: 0px 0px 6px #C2C2C2;'>
			...
			<!-- SCAN -->
			<?php if ( $view != 'deleted' ) { ?>
				<a id='wpmc_scan' onclick='wpmc_scan()' class='button-primary' style='float: left;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-file-cleaner/img'); ?>search.png' /><?php _e("Scan", 'media-file-cleaner'); ?></a>
			<?php } ?>

			<!-- DELETE SELECTED -->
			<a id='wpmc_delete' onclick='wpmc_delete()' class='button' style='float: left; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-file-cleaner/img'); ?>delete.png' /><?php _e("Delete", 'media-file-cleaner'); ?></a>
			<?php if ( $view == 'deleted' ) { ?>
				<a id='wpmc_recover' onclick='wpmc_recover()' class='button-secondary' style='float: left; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-file-cleaner/img'); ?>pills.png' /><?php _e("Recover", 'media-file-cleaner'); ?></a>
			<?php } ?>

			<!-- IGNORE SELECTED -->
			<a id='wpmc_ignore' onclick='wpmc_ignore()' class='button' style='float: left; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-file-cleaner/img'); ?>tick.png' /><?php _e("Ignore", 'media-file-cleaner'); ?></a>

			<!-- RESET -->
			<?php if ( $view != 'deleted' ) { ?>
				<a id='wpmc_reset' href='?page=media-file-cleaner&reset=1' class='button-primary' style='float: right; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-file-cleaner/img'); ?>burn.png' /><?php _e("Reset", 'media-file-cleaner'); ?></a>
			<?php } ?>

			<!-- DELETE ALL -->
			<?php if ( $view == 'deleted' ) { ?>
				<a id='wpmc_recover_all' onclick='wpmc_recover_all()' class='button-primary' style='float: right; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-file-cleaner/img'); ?>pills.png' /><?php _e("Recover all", 'media-file-cleaner'); ?></a>
				<a id='wpmc_delete_all' onclick='wpmc_delete_all(true)' class='button button-red' style='float: right; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-file-cleaner/img'); ?>trash.png' /><?php _e("Empty trash", 'media-file-cleaner'); ?></a>
			<?php } else { ?>
				<a id='wpmc_delete_all' onclick='wpmc_delete_all()' class='button button-red' style='float: right; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-file-cleaner/img'); ?>delete.png' /><?php _e("Delete all", 'media-file-cleaner'); ?></a>
			<?php } ?>
			...
			<form id="posts-filter" action="upload.php" method="get" style='float: right;'>
				<p class="search-box" style='margin-left: 5px; float: left;'>
					...
					<input type="hidden" name="page" value="media-file-cleaner">
					<input type="hidden" name="view" value="<?php echo $view; ?>">
					<input type="hidden" name="paged" value="<?php echo $paged; ?>">
					<input type="submit" class="button" value="<?php _e( 'Search', 'media-file-cleaner' ) ?>"><span style='border-right: #A2A2A2 solid 1px; margin-left: 5px; margin-right: 3px;'>&nbsp;</span>
				</p>
			</form>
		...
		<p>
			<?php
				...
				echo "<p>";
				_e( "<b>Backup your database and files before delete! This is important.</b> The files deleted by the plugin will be moved to the 'uploads/wpmc-trash' directory and you can trash them later. I am trying my best to make this plugin safe but since it deletes files on a very dynamic system (= WordPress), it is impossible to predict every single situation. Be careful.", 'media-file-cleaner' );
				if ( $scan_media )
					_e( " If you delete an item of the type MEDIA, the database entry for it (Media Library) will be deleted permanently.", 'media-file-cleaner' );
				echo "</p>";

				if ( !$scan_files && !$scan_media ) {
					echo "<div class=\"error\"><p>";
					_e( "Scan is not enabled for neither the files or for the medias. The plugin is basically disabled. <b>Please check Settings > Media Cleaner</b>.", 'media-file-cleaner' );
					echo "</p></div>";
				}
				else if ( !wpmc_is_pro() ) {
					echo "<div class=\"updated\"><p>";
					_e( "<b>This version is not Pro.</b> It will only let you delete the files <u><b>one by one</b></u>, for your safety. <b>If the plugin works perfectly for deleting the files one by one then you can <a href='http://apps.meow.fr/media-file-cleaner'>get the PRO version</a></b>. It will let you select all files in one click for one page and also to use the <b>Delete all</b> function.", 'media-file-cleaner' );
					echo "</p></div>";
				}

				echo sprintf( __( 'There are <b>%d issue(s)</b> with your files, accounting for <b>%d MB</b>. Your trash contains <b>%d MB.</b>', 'media-file-cleaner' ), $issues_count, number_format( $total_size / 1000000, 2 ), number_format( $trash_total_size / 1000000, 2 ) );
			?>
		</p>
		...
		echo paginate_links(array(
			'base' => '?page=media-file-cleaner&s=' . urlencode($s) . '&view=' . $view . '%_%',
			'current' => $paged,
			'format' => '&paged=%#%',
		...
		<ul class="subsubsub">
			<li class="all"><a <?php if ( $view == 'issues' ) echo "class='current'"; ?> href='?page=media-file-cleaner&s=<?php echo $s; ?>&view=issues'><?php _e( "Issues", 'media-file-cleaner' ); ?></a><span class="count">(<?php echo $issues_count; ?>)</span></li> |
			<li class="all"><a <?php if ( $view == 'ignored' ) echo "class='current'"; ?> href='?page=media-file-cleaner&s=<?php echo $s; ?>&view=ignored'><?php _e( "Ignored", 'media-file-cleaner' ); ?></a><span class="count">(<?php echo $ignored_count; ?>)</span></li> |
			<li class="all"><a <?php if ( $view == 'deleted' ) echo "class='current'"; ?> href='?page=media-file-cleaner&s=<?php echo $s; ?>&view=deleted'><?php _e( "Trash", 'media-file-cleaner' ); ?></a><span class="count">(<?php echo $deleted_count; ?>)</span></li>
		</ul>
		...
		<table id='wpmc-table' class='wp-list-table widefat fixed media'>
			...
			<thead>
				<tr>
					...
					<th style='width: 64px;'><?php _e( 'Thumb', 'media-file-cleaner' ) ?></th>
					<?php endif; ?>
					<th style='width: 50px;'><?php _e( 'Type', 'media-file-cleaner' ) ?></th>
					<th style='width: 80px;'><?php _e( 'Origin', 'media-file-cleaner' ) ?></th>
					<th><?php _e( 'Path', 'media-file-cleaner' ) ?></th>
					<th style='width: 220px;'><?php _e( 'Issue', 'media-file-cleaner' ) ?></th>
					<th style='width: 80px; text-align: right;'><?php _e( 'Size', 'media-file-cleaner' ) ?></th>
				</tr>
			</thead>
			...
			<tfoot>
				...
				<th></th>
				<?php endif; ?>
				<th><?php _e( 'Type', 'media-file-cleaner' ) ?></th><th><?php _e( 'Origin', 'media-file-cleaner' ) ?></th><th><?php _e( 'Path', 'media-file-cleaner' ) ?></th><th><?php _e( 'Issue', 'media-file-cleaner' ) ?></th><th style='width: 80px; text-align: right;'><?php _e( 'Size', 'media-file-cleaner' ) ?></th></tr>
			</tfoot>
...
function wpmc_admin_menu() {
	load_plugin_textdomain( 'media-file-cleaner', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	add_media_page( 'Media Cleaner', 'Media Cleaner', 'manage_options', 'media-file-cleaner', 'wpmc_screen' );
...
function wpmc_activate () {
	...
	if ( !is_writable( $basedir ) ) {
		echo '<div class="error"><p>' . __( 'The directory for uploads is not writable. Media File Cleaner will only be able to scan.', 'media-file-cleaner' ) . '</p></div>';
	}
}
...
function wpmc_wp_enqueue_scripts () {
	wp_enqueue_style( 'media-file-cleaner-css', plugins_url( '/media-file-cleaner.css', __FILE__ ) );
	wp_enqueue_script( 'media-file-cleaner', plugins_url( '/media-file-cleaner.js', __FILE__ ), array( 'jquery' ), "3.0.4", true );
}
